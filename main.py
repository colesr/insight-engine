...
def fetch_worldbank(
    ...
    for ind_id in ind_list:
        ...
        try:
            ...
            if len(data) < 2:
                continue
            # Keywords that strongly indicate World Bank aggregate/region names
            AGGREGATE_KEYWORDS = ["(excluding", "IDA", "IBRD", "income levels", "all income", "small states", "demographic dividend", "OECD", "World", "Arab World", "European Union", "Caribbean", "Central Europe", "East Asia & Pacific", "Europe & Central Asia", "Latin America &", "Middle East &", "North America", "South Asia", "Sub-Saharan", "Fragile", "Heavily indebted", "Least developed"]
            for item in data[1]:
                ...
                if not country or value is None:
                    continue
                # Skip region/aggregate entries by checking for aggregate keywords in the raw name
                is_aggregate = any(kw in country_raw for kw in AGGREGATE_KEYWORDS)
                if is_aggregate:
                    continue
