...
async function fetchWorldBankData() {
  ...
  showStatus('Fetching World Bank data...', 30);
  try {
    const result = await api(`/api/worldbank/fetch?indicators=${selected.join(',')}&countries=all&date_range=2020:2023`);
    if (!result.data || !result.data.length) { showStatus('No data returned', 100); return; }
    const keys = Object.keys(result.data[0]).filter(x => x !== 'country');
...
async function fetchClimateData() {
  showStatus('Fetching climate data...', 30);
  try {
    const result = await api('/api/climate/fetch');
