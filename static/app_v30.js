...
function drawGlobeGrid(radius) {
  const gridMat = new THREE.LineBasicMaterial({ color: 0x2a4a7a, transparent: true, opacity: 0.35, depthTest: true });
  const equatorMat = new THREE.LineBasicMaterial({ color: 0x38bdf8, transparent: true, opacity: 0.45, depthTest: true });
  const primeMat = new THREE.LineBasicMaterial({ color: 0x38bdf8, transparent: true, opacity: 0.35, depthTest: true });
...
function drawCountryBoundaries(radius) {
  ...
  // Shared geometry and material for coastline dots (instanced for performance)
  const dotGeo = new THREE.SphereGeometry(0.7, 5, 5);
  ...
  const glowGeo = new THREE.SphereGeometry(1.4, 5, 5);
  ...
  coastlines.forEach(chain => {
    ...
    // Place dots at every 2nd point for dense but visible coastline
...
function initGlobe() {
  ...
  // Base globe — dark sphere with subtle grid
  ...
  // Very subtle wireframe overlay — minimal, just enough to show sphere form
  const wireGeo = new THREE.SphereGeometry(R + 0.2, 12, 12);
  ...
  // Atmosphere glow (slightly larger, backside rim)
  ...
  const atmo2Geo = new THREE.SphereGeometry(R + 6, 64, 64);
